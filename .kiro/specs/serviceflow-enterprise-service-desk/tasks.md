...
- [ ] 9. Implement `SlaService` and `BusinessHoursCalculator`
  - [-] 9.1 Implement `BusinessHoursCalculator` — calculate elapsed business minutes between two timestamps given a schedule (weekday hours, excluded holidays)
    - _Requirements: 3.3_
  ...
  - [-] 9.3 Implement `SlaService::assignPolicy` — on ticket creation, match ticket priority to `SlaPolicy`, create `SlaTimer` record
    - _Requirements: 3.1_
  ...
  - [-] 9.5 Implement `SlaService::recordFirstResponse` — stamp `first_response_at` on `SlaTimer` when first agent comment is posted
    - _Requirements: 3.2_
  ...
  - [-] 9.7 Implement `SlaService::checkBreach` — compare elapsed business hours against policy thresholds, set `breached = true` when exceeded
    - _Requirements: 3.4_
  ...
  - [-] 9.9 Implement `SlaEscalationJob` — scheduled job that calls `SlaService::checkBreach` for all open timers and dispatches `SlaBreached` event
    - Register in `Console/Kernel` schedule (every minute)
    - _Requirements: 3.4, 3.5_

- [~] 10. Checkpoint — SLA engine
...
- [ ] 11. Implement inbound email pipeline
  - [~] 11.1 Implement `EmailParser` — parse raw RFC 2822 email into structured DTO (from, subject, body, attachments, message-id, in-reply-to)
    - _Requirements: 4.1_
  - [~] 11.2 Implement `EmailToTicketAction` — if `in-reply-to` matches existing `EmailThread.message_id`, append comment; otherwise create new ticket
  ...
  - [~] 11.4 Implement `IngestEmailCommand` (`email:ingest`) — read raw email from stdin, call `EmailParser` then `EmailToTicketAction`
    - _Requirements: 4.2_
  - [~] 11.5 Implement cPanel pipe script stub (`scripts/pipe.php`) and document `.forward` configuration
    - _Requirements: 4.2_

- [ ] 12. Implement outbound email (`TicketMailer`)
  - [~] 12.1 Implement `TicketMailer` — send ticket created, comment added, status changed, and SLA breach notifications using Laravel Mail
...
- [~] 13. Checkpoint — Email integration
...
- [ ] 14. Implement `ArticleService` and `ArticleSearchService`
  - [~] 14.1 Implement `ArticleService` — CRUD for `KnowledgeArticle` with draft/published status, slug generation, category assignment
