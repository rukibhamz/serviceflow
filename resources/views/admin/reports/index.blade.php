...
<div class="stats-ds mb-6">
    <div class="stat-card"><div class="stat-label">Created This Month</div><div class="stat-val text-brand">{{ $thisMonth }}</div><div class="text-xs text-gray-400 mt-1">{{ $lastMonth }} last month</div></div>
    <div class="stat-card"><div class="stat-label">Resolved This Month</div><div class="stat-val text-green-600">{{ $resolvedMonth }}</div></div>
    <div class="stat-card"><div class="stat-label">Avg Resolution Time</div><div class="stat-val text-blue-600">{{ $avgResolution ? round($avgResolution).'h' : '—' }}</div></div>
    <div class="stat-card"><div class="stat-label">SLA Breaches</div><div class="stat-val {{ $slaBreached > 0 ? 'text-red-500' : 'text-gray-400' }}">{{ $slaBreached }}</div></div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
    ...
    <div class="card-ds">
        ...
        <div class="card-body space-y-2">
            ...
            <div>
                <div class="flex justify-between text-xs text-gray-600 mb-0.5"><span>{{ ucfirst(str_replace('_',' ',$s)) }}</span><span class="font-medium">{{ $count }}</span></div>
                <div class="w-full bg-gray-100 rounded-full h-1.5"><div class="h-1.5 rounded-full bg-brand" style="width:{{ round($count/$total*100) }}%"></div></div>
            </div>
            @endforeach
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                <div class="flex items-center gap-2">
                    <div class="w-7 h-7 rounded-full bg-accent flex items-center justify-center text-white text-xs font-semibold">{{ strtoupper(substr($row->assignee?->name ?? 'U', 0, 2)) }}</div>
...
(function() {
    new Chart(document.getElementById('volumeChart'), {
        ...
        data: {
            ...
            datasets: [{ label: 'Tickets Created', data: {!! json_encode($days->values()) !!}, backgroundColor: 'rgba(26,79,160,0.15)', borderColor: '#1a4fa0', borderWidth: 2, borderRadius: 4 }]
        },
    ...
    new Chart(document.getElementById('priorityChart'), {
        ...
        data: {
            ...
            datasets: [{ data: {!! json_encode($byPriority->values()) !!}, backgroundColor: ['#ef4444','#f97316','#eab308','#22c55e','#94a3b8'], borderWidth: 2 }]
        },
