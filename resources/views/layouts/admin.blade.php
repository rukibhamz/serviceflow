...
<head>
    ...
    <title>{{ app(\App\Services\SettingService::class)->get('brand_name', config('app.name', 'ServiceFlow')) }} — Admin</title>
...
<div class="app-shell" x-data="{ sidebarOpen: window.innerWidth > 768, profileOpen: false }"
    ...
    <div class="topnav">
        ...
        @if (!empty($appSettings['brand_logo']))
            <a href="{{ route('admin.dashboard') }}" class="flex items-center">
                <img src="{{ $brandLogoUrl }}" alt="{{ $appSettings['brand_name'] ?? 'ServiceFlow' }}" class="h-7 max-w-[160px] object-contain">
            </a>
        @else
            <div class="logo"><div class="logo-dot"></div>{{ $appSettings['brand_name'] ?? config('app.name', 'ServiceFlow') }}</div>
        @endif
        ...
        <div class="relative ml-auto" x-data>
            ...
            <div x-show="profileOpen" @click.outside="profileOpen = false"
                ...
                <a href="{{ route('admin.profile') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-gray-50 text-gray-700">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                    My Profile
                </a>
                <div class="border-t border-gray-100 mt-1 pt-1">
                    <a href="{{ route('logout') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-red-50 text-red-500">
    ...
    <div class="body-wrapper">
        ...
        <div class="sidebar-shell"
            ...
            <div x-show="!sidebarOpen"
                ...
                @php $isActive = $item['match'] ? Request::routeIs($item['match']) : false; @endphp
                <a href="{{ route($item['route']) }}" title="{{ $item['title'] }}"
                   style="display:flex;align-items:center;justify-content:center;width:44px;height:44px;border-radius:6px;background:{{ $isActive ? 'rgba(255,255,255,0.1)' : 'transparent' }};color:{{ $isActive ? '#fff' : 'rgba(255,255,255,0.6)' }};text-decoration:none;transition:background 0.15s;">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor">{!! $item['icon'] !!}</svg>
                </a>
                @endforeach
            </div>
            ...
            <div class="sidebar-nav" x-show="sidebarOpen"
                ...
                <a href="{{ route('admin.tickets.index') }}" class="sidebar-item {{ Request::routeIs('admin.tickets.index') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.tickets.kanban') }}" class="sidebar-item {{ Request::routeIs('admin.tickets.kanban') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.changes.index') }}" class="sidebar-item {{ Request::routeIs('admin.changes.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.problems.index') }}" class="sidebar-item {{ Request::routeIs('admin.problems.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.teams') }}" class="sidebar-item {{ Request::routeIs('admin.teams') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.tenants') }}" class="sidebar-item {{ Request::routeIs('admin.tenants') ? 'active' : '' }}">
                    <svg class="sidebar-icon" viewBox="0 0 16 16" fill="none" stroke="currentColor"><rect x="2" y="3" width="12" height="10" rx="1.5" stroke-width="1.2"/><path d="M5 7h6M5 9h4" stroke-width="1.2" stroke-linecap="round"/></svg>
                    Tenants
                </a>

                <div class="sidebar-section">Service Management</div>
                <a href="{{ route('admin.sla') }}" class="sidebar-item {{ Request::routeIs('admin.sla') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.automation.index') }}" class="sidebar-item {{ Request::routeIs('admin.automation.*') ? 'active' : '' }}">
                ...
                <a href="{{ route('agent.dashboard') }}" class="sidebar-item">
