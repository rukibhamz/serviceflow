...
<head>
    ...
    {{-- Dynamic theme override from admin branding settings --}}
    ...
    <div class="app-shell" x-data="{ sidebarOpen: window.innerWidth > 768, profileOpen: false }" @resize.window="if (window.innerWidth > 768) sidebarOpen = true; else sidebarOpen = false;">
        ...
        <div class="topnav">
            ...
            @if (!empty($appSettings['brand_logo']))
                <a href="{{ route('agent.dashboard') }}" class="flex items-center">
            ...
            <div class="relative ml-auto" x-data>
                ...
                >{{ strtoupper(substr(auth()->user()?->name ?? 'GU', 0, 2)) }}</button>
                ...
                >
                    ...
                    <a href="{{ route('agent.profile') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-gray-50 text-gray-700">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        My Profile
                    </a>
                    <a href="{{ route('agent.settings.index') }}" class="flex items-center gap-2 px-4 py-2 hover:bg-gray-50 text-gray-700">
        ...
        <div class="body-wrapper">
            ...
            <div class="sidebar-shell" :class="sidebarOpen ? 'sidebar-expanded' : 'sidebar-collapsed'"
                ...
                {{-- COLLAPSED STATE: icon strip with expand arrow + nav icons --}}
                <div x-show="!sidebarOpen"
                    ...
                    @foreach($stripItems as $item)
                        ...
                        <a href="{{ route($item['route']) }}" title="{{ $item['title'] }}"
                           style="display:flex;align-items:center;justify-content:center;width:44px;height:44px;border-radius:6px;background:{{ $bg }};color:{{ $color }};text-decoration:none;transition:background 0.15s,transform 0.12s;flex-shrink:0;border-left:{{ $borderLeft }};"
                           ...
                           onmouseout="if(this.dataset.active!='1'){this.style.background='{{ $bg }}';this.style.color='{{ $color }}';}this.style.transform='scale(1)'"
                           data-active="{{ $isActive ? '1' : '0' }}">
                            <svg width="20" height="20" viewBox="0 0 16 16" fill="none" stroke="currentColor">{!! $item['icon'] !!}</svg>
                        </a>
                    @endforeach
                </div>

                {{-- EXPANDED STATE: full nav --}}
                <div class="sidebar-nav" x-show="sidebarOpen"
                    ...
                    {{-- "SERVICE DESK" header with collapse arrow (←) --}}
                    ...
                    <a href="{{ route('agent.tickets.triage') }}" class="sidebar-item {{ Request::routeIs('agent.tickets.triage') ? 'active' : '' }}">
                        ...
                        My Queue
                        @php $myQueueCount = \App\Models\Ticket::whereNull('assignee_id')->whereNotIn('status', ['resolved','closed'])->count(); @endphp
                        @if($myQueueCount > 0)
                    ...
                    <a href="{{ route('agent.settings.index') }}" class="sidebar-item {{ Request::routeIs('agent.settings.index') ? 'active' : '' }}">
