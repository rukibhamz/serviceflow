...
<head>
    ...
    <title>Accept Invitation — {{ app(\App\Services\SettingService::class)->get('brand_name', config('app.name', 'ServiceFlow')) }}</title>
...
<div class="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
    <div class="mb-6 text-center">
        ...
        <p class="text-sm text-gray-500 mt-1">You've been invited as a <strong>{{ ucfirst($invitation->role) }}</strong></p>
        <p class="text-xs text-gray-400 mt-1">{{ $invitation->email }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('invite.accept', $invitation->token) }}" class="space-y-4">
        ...
        <div>
            ...
            <div class="relative" x-data="{ show: false }">
                <input :type="show ? 'text' : 'password'" name="password" required
                       class="w-full border border-gray-300 rounded-xl px-4 py-3 pr-11 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <button type="submit"
                class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-colors">
