                ...
                <form method="POST" action="{{ route('admin.teams.members.update', $selectedTeamId) }}">
                    ...
                    <div class="max-h-96 overflow-y-auto space-y-2 pr-2">
                        <div class="mb-3 rounded border border-gray-200 p-3">
                            <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Team Lead</label>
                            <select name="team_lead_id" class="w-full rounded border border-gray-300 px-3 py-2 text-sm">
                                <option value="">No team lead</option>
                                @foreach($teamLeadUsers as $leadUser)
                                    <option value="{{ $leadUser->id }}" @selected($selectedTeamLeadId === $leadUser->id)>{{ $leadUser->name }} ({{ $leadUser->email }})</option>
                                @endforeach
                            </select>
                        </div>
