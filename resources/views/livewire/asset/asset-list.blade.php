    ...
    <div class="bg-white border border-indigo-200 rounded-xl p-5 shadow-sm space-y-3">
        ...
        @foreach ($assigneeResults as $user)
            <div class="flex items-center justify-between py-1 border-b border-gray-100">
                ...
                <button wire:click="assignTo({{ $user->id }})" class="text-xs text-indigo-600 hover:underline">Assign</button>
            </div>
        @endforeach
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 text-right space-x-2">
                        <button wire:click="openAssign({{ $asset->id }})" class="text-xs text-indigo-600 hover:underline">Assign</button>
                        @if ($asset->assigned_to)
                            <button wire:click="unassign({{ $asset->id }})" class="text-xs text-orange-500 hover:underline">Unassign</button>
                        @endif
                        <button wire:click="edit({{ $asset->id }})" class="text-xs text-gray-500 hover:underline">Edit</button>
                        <button wire:click="delete({{ $asset->id }})" wire:confirm="Delete this asset?" class="text-xs text-red-500 hover:underline">Delete</button>
                    </td>
                </tr>
                @empty
                <tr>
