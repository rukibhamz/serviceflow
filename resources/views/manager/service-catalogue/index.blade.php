...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        ...
        <a href="{{ route('manager.service-catalogue.create') }}" class="btn-ds primary">+ New Item</a>
    </div>
                ...
                <tr class="border-t border-slate-100">
                    ...
                    <td class="px-4 py-2">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('manager.service-catalogue.edit', $item) }}" class="btn-ds ghost">Edit</a>
                            <form method="POST" action="{{ route('manager.service-catalogue.toggle', $item) }}">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn-ds ghost">{{ $item->is_active ? 'Deactivate' : 'Activate' }}</button>
                            </form>
                            <form method="POST" action="{{ route('manager.service-catalogue.destroy', $item) }}" onsubmit="return confirm('Delete this catalogue item?');">
                                @csrf
                                @method('DELETE')
