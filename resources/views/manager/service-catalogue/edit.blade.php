...
@section('content')
    <div class="mb-4 flex items-center justify-between">
        ...
        <a href="{{ route('manager.service-catalogue.index') }}" class="btn-ds ghost">Back</a>
    </div>

    <form method="POST" action="{{ route('manager.service-catalogue.update', $item) }}" class="space-y-4 rounded border border-slate-200 bg-white p-4">
        @csrf
        @method('PATCH')
